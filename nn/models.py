...
class LSTMFeatureExtractor(nn.Module):
    def __init__(self, seq_len=1024, hidden_size=256, num_layers=5, num_classes=4,
        ...
        if not image:
            self.conv1 = nn.Conv1d(in_channels=in_channels, out_channels=channels, kernel_size=kernel_size,
                                   padding='same', stride=1)
            self.pool = nn.MaxPool1d(kernel_size=stride)
            self.skip = nn.Conv1d(in_channels=in_channels, out_channels=channels, kernel_size=1, padding=0,
                                  stride=stride)
            self.drop = nn.Dropout1d(p=dropout)
            self.batchnorm1 = nn.BatchNorm1d(channels)
        else:
            self.conv1 = nn.Conv2d(in_channels=in_channels, out_channels=channels, kernel_size=kernel_size,
                                   padding='same', stride=1)
            self.pool = nn.MaxPool2d(kernel_size=stride)
            self.skip = nn.Conv2d(in_channels=in_channels, out_channels=channels, kernel_size=1, padding=0,
                                  stride=stride)
        ...
        self.lstm = nn.LSTM(channels, hidden_size, num_layers=num_layers, batch_first=True, bidirectional=True,
                            dropout=dropout)
    ...
    def forward(self, x, return_cell=False):
        ...
        x_f, (h_f, c_f) = self.lstm(
            x)  # [B, L//stride, 2*hidden_size], [B, 2*num_layers, hidden_size], [B, 2*num_layers, hidden_size
        if return_cell:
            return x_f, h_f, c_f
        h_f = h_f.transpose(0, 1).transpose(1, 2)
        h_f = h_f.reshape(h_f.shape[0], -1)
...
class LSTM(nn.Module):
    def __init__(self, seq_len=1024, hidden_size=64, num_layers=5, num_classes=4,
        ...
        self.activation = nn.GELU()
        self.feature_extractor = LSTMFeatureExtractor(seq_len=seq_len, hidden_size=hidden_size, num_layers=num_layers,
...
class LSTM_DUAL(nn.Module):
    ...
    def forward(self, x, x_dual=None, acf_phr=None, return_features=False):
        ...
        if len(x.shape) == 2:
            x = x.unsqueeze(1)
        x, h_f, c_f = self.feature_extractor(x,
                                             return_cell=True)  # [B, L//stride, 2*hidden_size], [B, 2*nlayers, hidden_szie], [B, 2*nlayers, hidden_Size]
...
class SpotNet(torch.nn.Module):
    ...
    def forward(self, x_acf, x):
        ...
        tic = time.time()
        x_f, h_f, c_f = self.feature_extractor(x_acf,
                                               return_cell=True)  # [B, L//stride, 2*hidden_size], [B, 2*nlayers, hidden_szie], [B, 2*nlayers, hidden_Size]
        c_f = torch.cat([c_f[-1], c_f[-2]], dim=1)  # [B, 2*hidden_szie]
