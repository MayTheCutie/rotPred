...
def high_acovf(x, adjusted=False, demean=True, fft=True, missing="none", nlag=None):
    """
    Estimate autocovariances.
    ...
    if fft:
        nobs = len(xo)
        n = _next_regular(2 * nobs + 1)
        Frf = np.fft.fft2(xoo, s=[n,n])
        acov = np.fft.ifft2(Frf * np.conjugate(Frf))[:nobs, :nobs] / d[nobs - 1 :, None]
        acov = acov.real
    else:
        acov = np.correlate(xo, xo, "full")[n - 1 :] / d[n - 1 :]
    ...
    if nlag is not None:
        # Copy to allow gc of full array rather than view
        return acov[: lag_len + 1, : lag_len + 1].copy()
    return acov
